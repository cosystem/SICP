...
;(display (deriv_getchanged '(+ x 3) 'x))
; will give error
(display "\n---- test deriv_getchanged agains put no change finish ----")

;; ---- test with both put and get in changed order -----

(define (deriv_getandputchanged exp var)
  (cond ((number? exp) 0)
        ((variable? exp) (if (same-variable? exp var) 1 0))
        (else ((get (operator exp) 'deriv_opr) (operands exp) var))))

(define (install-sum-package_putchange)
  ;; internal procedure
  (define (deriv_opr oprand var)
    ;; the previous addend/augend is not valid anymore, as deriv will be applied on operands
    (make-sum (deriv_getandputchanged (car oprand) var)
              (deriv_getandputchanged(cadr oprand) var))) 
  ;; interface to the rest of the system
  (put '+ 'deriv_opr deriv_opr))

(define (install-product-package_putchange)
  ;; internal procedure
  (define (deriv_opr oprand var)
    (make-sum
     (make-product (car oprand)
                   (deriv_getandputchanged (cadr oprand) var))
     (make-product (deriv_getandputchanged (car oprand) var)
                   (cadr oprand))))
  ;; interface to the rest of the system
  (put '* 'deriv_opr deriv_opr))

(newline)
(display "\n---- test deriv_getchanged agains put change ----\n")
(install-sum-package_putchange)
(install-product-package_putchange)
(display (deriv_getchanged '(* (+ x 3) x) 'x))
(display "\n---- test deriv_getchanged agains put change finish ----")
