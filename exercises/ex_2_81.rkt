...
; test with original apply-generic
...
; add scheme number and complex number
(define (add x y) (apply-generic 'add x y))
(newline)
(display (add test_scheme_n z1))

;; Q 2.81 c.
;; Modify apply-generic so that it does not try coericion if the two arguments have the same type
              ...
              (let ((type1 (car type-tags))
                ...
                (if (not (eq? type1 type2))
                    (let ((t1->t2 (get-coercion type1 type2))
                          ...
                          (a2 (cadr args)))
                      (cond (t1->t2
                             (apply-generic-new op (t1->t2 a1) a2))
                            (t2->t1
                             (apply-generic-new op (t2->t1 a2)))
                            (else
                             (error "No method for these types" (list op type-tags)))))
                    (error "No method for these types" (list op type-tags))))
              (error "No method for these types" (list op type-tags)))))))
...
; add scheme number and complex number
(define (add-new x y) (apply-generic-new 'add x y))
(newline)
(display (add-new test_scheme_n z1))
