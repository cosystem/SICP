...
(define (install-polynomial-package)
  ...
  (define (order term termlist)
    (define (len l)
      (if (null? l)
          0
          (+ 1 (len (cdr l)))))
    (cond ((empty-termlist? termlist) (error "Term is not in term list -- ORDER" term))
          ((equ? term (car termlist)) (len termlist))
          (else (order term (cdr termlist)))))
...
(define p0 (make-polynomial 'x (list 14 5 2)))
(define p1 (make-polynomial 'x (list 3 0 1 6)))
